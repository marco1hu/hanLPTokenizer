...
def chat_with_deepseek(messages, model="deepseek-chat", temperature=0.7, max_tokens=512):
    ...
    try:
        ...
        logger.info(f"DeepSeek completato in {duration:.2f}s")
        return response.choices[0].message.content.strip()

    except APITimeoutError as e:
        logger.warning(f"Timeout DeepSeek: {e}")
    except Exception as e:
        logger.warning(f"Errore DeepSeek: {str(e)}")

    if fallback_enabled:
        logger.info("Fallback attivo → uso OpenAI GPT (gpt-3.5-turbo)")
        try:
            return chat_with_fallback(messages, model="gpt-3.5-turbo", temperature=temperature)
        except Exception as fallback_error:
            logger.exception("Errore anche nel fallback OpenAI")
